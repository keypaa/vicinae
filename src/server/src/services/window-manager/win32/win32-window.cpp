...
QString Win32Window::id() const { return QString::number(reinterpret_cast<quintptr>(m_hwnd), 16); }

QString Win32Window::title() const {
  wchar_t buffer[1024];
  int len = GetWindowTextW(m_hwnd, buffer, sizeof(buffer) / sizeof(wchar_t));
  if (len > 0) { return QString::fromWCharArray(buffer, len); }
  return {};
}
...
std::optional<int> Win32Window::pid() const {
  ...
  if (processId != 0) { return static_cast<int>(processId); }
  return std::nullopt;
}

std::optional<AbstractWindowManager::WindowBounds> Win32Window::bounds() const {
  RECT rect{};
  if (!GetWindowRect(m_hwnd, &rect)) { return std::nullopt; }
  return WindowBounds{
